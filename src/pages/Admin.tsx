...
const NavItem = ({
  ...
  const btn = (
    ...
    >
      ...
      />
      <span
        className={`transition-all duration-300 overflow-hidden whitespace-nowrap ${isCollapsed ? "w-0 opacity-0 hidden" : "w-auto opacity-100 block"
          }`}
      >
        {label}
      </span>
...
export default function Admin({ isDemoRoute }: { isDemoRoute?: boolean }) {
  ...
  // --- Gesture Navigation (Swipe to change tabs) ---
  const minSwipeDistance = 75;

  const onTouchStartSwipe = (e: React.TouchEvent) => {
    // Only detect swipes on the main content area, not on specific components that might have their own horizontal scroll
    if ((e.target as HTMLElement).closest('.overflow-x-auto, input, textarea')) return;
    setTouchEnd(null);
  ...
  const onTouchEndSwipe = () => {
    ...
    if (isLeftSwipe || isRightSwipe) {
      const flatNav = orderedNav.filter(n => categoryOrder.includes(n.group));
      const currentIndex = flatNav.findIndex(n => n.value === activeTab);
      if (currentIndex !== -1) {
        if (isLeftSwipe && currentIndex < flatNav.length - 1) {
          setActiveTab(flatNav[currentIndex + 1].value);
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex h-screen w-full overflow-hidden bg-background text-foreground pb-16 lg:pb-0">
              ...
              <div className="hidden lg:flex h-auto border-t border-border/40 shrink-0 flex-col gap-0">
                ...
                <div className="flex items-center justify-between px-4 h-12">
                  <button
                    ...
                    className={`flex items-center gap-1.5 p-1.5 rounded-md transition-colors text-xs ${reorderMode ? "bg-primary/20 text-primary" : "text-muted-foreground hover:bg-background-elevated hover:text-foreground"
                      }`}
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
              ...
              <main
                ...
                style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 112px)', touchAction: 'pan-y' }}
            ...
            <nav className="lg:hidden fixed bottom-4 left-4 right-4 z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
              <div className="relative bg-background/60 backdrop-blur-3xl border border-border/30 rounded-[28px] dock-glow flex items-center justify-around px-2 py-1" style={{ animation: 'dock-bounce-in 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>
                ...
                {[
                  ...
                  if (!item) return null;
                  const isActive = !item.isCenter && activeTab === item.value;
                  const Icon = item.icon;
                  const colorVar = item.colorClass?.includes("text-primary") ? "hsl(var(--primary))"
                    : item.colorClass?.includes("text-success") ? "hsl(var(--success))"
                      : item.colorClass?.includes("text-destructive") ? "hsl(var(--destructive))"
